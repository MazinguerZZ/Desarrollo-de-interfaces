# Práctica 2.1 — Sorpresita Javascript
...
**6.** Escribe el siguiente código, observa el resultado y coméntalo. Después repite el ejercicio declarando la variable correctamente:
```js
"use strict";
x = 10;
console.log(x);
...
**9.** Usando el operador de exponente `**`, calcula el cuadrado de un número, el cubo y la raíz cúbica.
...
**22.** Crea `frutas = ["manzana", "pera"]`. Muestra y comenta `typeof frutas`, `frutas instanceof Array` y `frutas instanceof Object`.

**23.** Crea `fecha = new Date()`. Muestra y comenta `typeof fecha`, `fecha instanceof Date` y `fecha instanceof Object`.
...
**28.** Declara `palabra = "JavaScript"`. Muestra el primer carácter, el último, la longitud con `.length` y `palabra[100]`. Comenta qué obtienes en cada caso.
